...
2    #_MSY: 1=F(SPR); 2=calc F(MSY); 3=set to F(Btgt) or F(0.1); 4=F(endyr) 
0.4  #_SPR target (e.g. 0.40)
0.342 #_Biomass target (e.g. 0.40)
...
1    #_Bmark_relF_Basis: 1 = use year range; 2 = set relF same as forecast below
#
1    #_Forecast: 0=none; 1=F(SPR); 2=F(MSY) 3=F(Btgt) or F0.1; 4=Ave F (uses first-last relF yrs); 5=input annual F scalar
10   #_N forecast years
0.2  #_F scalar (only used for Do_Forecast==5)
...
1    #_Forecast recruitment: 0=spawn_recr; 1=value*spawn_recr; 2=value*VirginRecr; 3=recent mean
1    #_Value ignored 
0    #_Forecast loop control #5 (reserved for future bells&whistles)
2010 #_First Year for caps and allocations (should be after years with fixed inputs)
...
1999 #_Rebuilder: first year catch could have been set to zero (Ydecl)(-1 to set to 1999)
2002 #_Rebuilder: year for current age structure (Yinit) (-1 to set to endyear+1)
